...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kudzucoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kudzucoin Signed Message:\n";
...
{
    // Kudzucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Kudzucoin: 840k blocks in ~4 years
...
static const int64 nTargetTimespan = 1.0 * 24 * 60 * 60; // Kudzucoin: 1.0 days
static const int64 nTargetSpacing = 2.0 * 60; // Kudzucoin: 2.0 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kudzucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // kudzucoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //kudzucoin, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kudzucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kudzucoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "04/22/2014 Ukraine parliament ousts president, sets elections for May";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Kudzucoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KudzucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KudzucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KudzucoinMiner(CWallet *pwallet)
{
    printf("KudzucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kudzucoin-miner");
    ...
    try { loop {
        ...
        printf("Running KudzucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KudzucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KudzucoinMiner, pwallet));
}
